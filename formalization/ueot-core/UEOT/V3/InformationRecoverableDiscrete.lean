...
theorem decoded_marginal_eq
  ...
  have hleft : (ρ.map (decodedCopyMap d)).snd = ρ.snd.map d := by
    ...
    rfl
  have hdiag : Measurable (fun c : C => (c, c)) :=
...
theorem product_reference_map_decodedCopy
  ...
  have hfun : decodedCopyMap d = Prod.map id d := by
    funext z
    rfl
  have hid : ρ.fst.map id = ρ.fst := by
    change ρ.fst.map (fun c : C => c) = ρ.fst
    exact Measure.map_id'
  calc
    (ρ.fst.prod ρ.snd).map (decodedCopyMap d) =
        (ρ.fst.prod ρ.snd).map (Prod.map id d) :=
      congrArg (fun f => (ρ.fst.prod ρ.snd).map f) hfun
    _ = (ρ.fst.map id).prod (ρ.snd.map d) := hmap.symm
    _ = ρ.fst.prod ρ.fst := by
      rw [hid, hmarg]
