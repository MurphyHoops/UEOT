...
def assemblePrefixFuture {N : ℕ} (i : Fin N)
    (x : prefixBlock (N := N) i.1 → H) (y : future i → H) : Fin N → H :=
  fun j =>
    if h : j.1 ≤ i.1 then
      x ⟨j, (mem_prefixBlock_iff i.1 j).2 h⟩
    else
      y ⟨j, (mem_future_iff i j).2 (Fin.lt_def.2 (Nat.lt_of_not_ge h))⟩
...
@[simp] theorem assemblePrefixFuture_future
  ...
  have hnle : ¬ j.1.1 ≤ i.1 := by
    exact Nat.not_le_of_lt (Fin.lt_def.1 hlt)
  simp [hnle]
...
theorem blockProjection_prefix_assemble
    {N : ℕ} (i : Fin N)
    (x : prefixBlock (N := N) i.1 → H) (y : future i → H) :
    blockProjection (prefixBlock (N := N) i.1) (assemblePrefixFuture i x y) = x := by
  funext j
...
theorem measurable_assemblePrefixFuture
    ...
    Measurable (fun xy : (prefixBlock (N := N) i.1 → H) × (future i → H) =>
      assemblePrefixFuture i xy.1 xy.2) := by
  rw [measurable_pi_iff]
  intro j
  by_cases h : j.1 ≤ i.1
  · simp only [assemblePrefixFuture, dif_pos h]
    let jp : prefixBlock (N := N) i.1 :=
      ⟨j, (mem_prefixBlock_iff i.1 j).2 h⟩
    exact (measurable_pi_apply jp).comp measurable_fst
  · simp only [assemblePrefixFuture, dif_neg h]
    let jf : future i :=
      ⟨j, (mem_future_iff i j).2 (Fin.lt_def.2 (Nat.lt_of_not_ge h))⟩
    exact (measurable_pi_apply jf).comp measurable_snd
