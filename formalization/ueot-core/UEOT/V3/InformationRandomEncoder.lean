...
/-- Forget the encoder output and recover the source sample `(U,H)` from the
reassociated randomized-encoder law. -/
def encoderSourceProjection : U × (H × M) → U × H :=
  fun z => (z.1, z.2.1)
...
lemma measurable_encoderSourceProjection :
    Measurable (encoderSourceProjection (U := U) (H := H) (M := M)) := by
  unfold encoderSourceProjection
...
/-- **Source-law invariance under randomized encoding.**  Adding a Markov
encoder output and reassociating the sample leaves the `(U,H)` source marginal
exactly equal to the original source law. -/
theorem randomEncoderJoint_map_sourceProjection
    (μ : Measure (U × H)) [IsProbabilityMeasure μ]
    (η : Kernel (U × H) M) [IsMarkovKernel η] :
    (randomEncoderJoint μ η).map
        (encoderSourceProjection (U := U) (H := H) (M := M)) = μ := by
  unfold randomEncoderJoint
  rw [Measure.map_map
    (measurable_encoderSourceProjection (U := U) (H := H) (M := M))
    (measurable_encoderReassoc (U := U) (H := H) (M := M))]
  simpa [Measure.fst, encoderSourceProjection, encoderReassoc, Function.comp_def] using
    (Measure.fst_compProd μ η)

/-- The side-information marginal `P(U)` is likewise invariant under adding a
randomized encoder output.  This is the outer measure used by all conditional
entropy and conditional mutual-information integrals. -/
theorem randomEncoderJoint_fst
    (μ : Measure (U × H)) [IsProbabilityMeasure μ]
    (η : Kernel (U × H) M) [IsMarkovKernel η] :
    (randomEncoderJoint μ η).fst = μ.fst := by
  unfold Measure.fst
  have hsource :
      (randomEncoderJoint μ η).map
          (encoderSourceProjection (U := U) (H := H) (M := M)) = μ :=
    randomEncoderJoint_map_sourceProjection μ η
  calc
    (randomEncoderJoint μ η).map Prod.fst =
        (randomEncoderJoint μ η).map
          (Prod.fst ∘ encoderSourceProjection (U := U) (H := H) (M := M)) := by
      apply Measure.map_congr
      filter_upwards with z
      rfl
    _ = ((randomEncoderJoint μ η).map
          (encoderSourceProjection (U := U) (H := H) (M := M))).map Prod.fst := by
      symm
      exact Measure.map_map measurable_fst
        (measurable_encoderSourceProjection (U := U) (H := H) (M := M))
    _ = μ.map Prod.fst := congrArg (fun ν : Measure (U × H) => ν.map Prod.fst) hsource
